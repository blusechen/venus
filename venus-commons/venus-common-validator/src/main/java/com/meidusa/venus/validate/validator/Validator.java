 ...
 * <p/>
 * <p>
 * FieldValidators (such as the EmailValidator) are designed to perform validation checks on a single field. They
 * require that you specify a fieldname attribute in your -validation.xml file. There are two different (but equivalent)
 ...
 * <p/>
 * <p>
 * <b>NOTE:</b>Note that you do not declare what "flavor" of validator you are using in your -validation.xml file, you
 * just declare the name of the validator to use and Struts will know whether it's a "plain Validator" or a
 ...
 * <p>
 * To define validation rules for an Action, create a file named ActionName-validation.xml in the same package as the
 * Action. You may also create alias-specific validation rules which add to the default validation rules defined in
 * ActionName-validation.xml by creating another file in the same directory named ActionName-aliasName-validation.xml.
 * In both cases, ActionName is the name of the Action class, and aliasName is the name of the Action alias defined in
 * the xwork.xml configuration for the Action.
 * </p>
 * <p/>
 * <p>
 * The framework will also search up the inheritance tree of the Action to find validation rules for directly
 * implemented interfaces and parent classes of the Action. This is particularly powerful when combined with ModelDriven
 * Actions and the VisitorFieldValidator. Here's an example of how validation rules are discovered. Given the following
 * class structure:
 * </p>
 * <p/>
 * <ul>
 ...
 * <p/>
 * <p>
 * While this process is similar to what the XW:Localization framework does when finding messages, there are some subtle
 * differences. The most important difference is that validation rules are discovered from the parent downwards.
 * </p>
 * <p/>
 * <p>
 * <b>NOTE:</b>Child's *-validation.xml will add on to parent's *-validation.xml according to the class hierarchy
 * defined above. With this feature, one could have more generic validation rule at the parent and more specific
 * validation rule at the child.
 * </p>
 * <p/>
 ...
 * <p/>
 * <p>
 * <b>Non-Field-Validator</b> The &lt;validator&gt; element allows you to declare both types of validators (either a
 ...
 * <p>
 * <b>field-validator</b> The &lt;field-validator&gt; elements are basically the same as the &lt;validator&gt; elements
 * except that they inherit the fieldName attribute from the enclosing &lt;field&gt; element. FieldValidators defined
 * within a &lt;field-validator&gt; element will have their fieldName automatically filled with the value of the parent
 * &lt;field&gt; element's fieldName attribute. The reason for this structure is to conveniently group the validators
 * for a particular field under one element, otherwise the fieldName attribute would have to be repeated, over and over,
 * for each individual &lt;validator&gt;.
 * </p>
 * <p/>
 * <p>
 * <b>HINT:</b> It is always better to defined field-validator inside a &lt;field&gt; tag instead of using a
 * &lt;validator&gt; tag and supplying fieldName as its param as the xml code itself is clearer (grouping of field is
 * clearer)
 * </p>
 * <p/>
 * <p>
 * <b>NOTE:</b> Note that you should only use FieldValidators (not plain Validators) within a <field-validator> block. A
 * plain Validator inside a &lt;field&gt; will not be allowed and would generate error when parsing the xml, as it is
 * not allowed in the defined dtd (xwork-validator-1.0.2.dtd)
 * </p>
 ...
 * <p>
 * The choice is yours. It's perfectly legal to only use <validator> elements without the <field> elements and set the
 * fieldName attribute for each of them. The following are effectively equal:
 * </P>
 ...
 * <p>
 * It is possible to short-circuit a stack of validators. Here is another sample config file containing validation rules
 * from the Xwork test cases: Notice that some of the &lt;field-validator&gt; and &lt;validator&gt; elements have the
 * short-circuit attribute set to true.
 * </p>
 ...
 * <p>
 * <b>short-circuiting and Validator flavors</b>
 * </p>
 * <p>
 * Plain validator takes precedence over field-validator. They get validated first in the order they are defined and
 * then the field-validator in the order they are defined. Failure of a particular validator marked as short-circuit
 * will prevent the evaluation of subsequent validators and an error (action error or field error depending on the type
 * of validator) will be added to the ValidationContext of the object being validated.
 * </p>
 * <p/>
 * <p>
 ...
 * <p/>
 * <p>
 * <b>Usefull Information:</b> More complicated validation should probably be done in the validate() method on the
 * action itself (assuming the action implements Validatable interface which ActionSupport already does).
 * </p>
 * <p/>
 * <p>
 ...
 * <p>
 * <b>Short cuircuiting and validator flavours</b>
 * </p>
 * <p>
 ...
 * FieldValidator's and will not short-circuit the plain ExpressionValidator because FieldValidators only short-circuit
 * other checks on that same field. Since the plain Validator is not field specific, it is not short-circuited.
 * </p>
 ...
 * validators higher up in the inheritance tree, make sure you don't accidentally short-circuit things higher in the
 * tree that you really want!
 * </p>
 * <p>
 ...
 * &lt;actionClass&gt;-validation.xml and more context specific validators to be located in
 * &lt;actionClass&gt;-&lt;actionAlias&gt;-validation.xml
 * </p>
 * <!-- END SNIPPET: howXworkFindsValidatorForAction -->
 * 
 * <p/>
 * <!-- START SNIPPET: i18n --> Validator's validation messages could be internatinalized. For example,
 * 
 * <pre>
 *   &lt;field-validator type="required"&gt;
 ...
 * <p/>
 * We could also provide a default message such that if validation failed and the i18n key for the message cannot be
 * found, WebWork would fall back and use the default message. An example would be as follows :-
 * 
 * <pre>
 *   &lt;field-validator type="required"&gt;
...
public interface Validator {
    ...
    /**
     * The validation implementation must guarantee that setValidatorContext will be called with a non-null
