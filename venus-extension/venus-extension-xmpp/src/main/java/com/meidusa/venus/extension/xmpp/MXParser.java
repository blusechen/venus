...
/**
 * MXParser that returns an IGNORABLE_WHITESPACE event when a whitespace character or a line feed is received. This
 * parser is useful when not validating documents.
 * 
...
public class MXParser extends org.xmlpull.mxp1.MXParser {
    ...
    /**
     * Last time a heartbeat was received. Hearbeats are represented as whitespaces or \n characters received when an
     * XmlPullParser.END_TAG was parsed. Note that we can falsely detect heartbeats when parsing XHTML content but that
     * is fine.
     */
                ...
                if (ch == '<') {
                    ...
                    do {
                        ...
                        if (eventType == XmlPullParser.END_TAG && (ch == ' ' || ch == '\n' || ch == '\t')) {
                            ...
                            lastHeartbeat = TimeUtil.currentTimeMillis();
                            ;
                        }
                        if (ch == ']') {
                            if (seenBracket) {
     ...
     * received when an XmlPullParser.END_TAG was parsed. Note that we can falsely detect heartbeats when parsing XHTML
     * content but that is fine.
     * 
    ...
    protected char more() throws IOException, XmlPullParserException {
        final char codePoint = super.more(); // note - this does NOT return a codepoint now, but simply a (single byte)
                                             // character!
        if ((codePoint == 0x0)
                || // 0x0 is not allowed, but flash clients insist on sending this as the very first character of a
                   // stream. We should stop allowing this codepoint after the first byte has been parsed.
                (codePoint == 0x9) || (codePoint == 0xA) || (codePoint == 0xD) || ((codePoint >= 0x20) && (codePoint <= 0xD7FF))
